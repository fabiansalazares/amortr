...
// amortr::fixed_german(10000, 0.01, as.Date("2020-01-01"), NULL, 20, 12, NULL)
// amortr::fixed_german(10000, 0.01, as.Date("2020-01-01"), NULL, 20, 12, c("hola"="yes"))
// amortr::fixed_german(10000, 0.01, as.Date("2020-01-01"), as.Date("2022-01-01"), NULL, 12, NULL)
// amortr::fixed_german(10000, 0.01, as.Date("2020-01-01"), as.Date("2022-01-01"), NULL, 12,  c("hola"="yes"))
...
//' Fixed-rate german loan amortization schedule calculation function.
 ...
 Rcpp::DataFrame fixed_german_(
   ...
   double principal_payment_per_period = amount/total_periods;
   
   interest_payment[0] = std::round(interest_rate*balance[0] / 0.01) * 0.01;
   principal_payment[0] = principal_payment_per_period;
   balance[0] = amount;
   total_payment[0] = principal_payment_per_period + interest_payment[0];
   
   for (int index=1; index < (total_periods - 1); index++) {
     principal_payment[index] = std::round(principal_payment_per_period/0.01) * 0.01;
     balance[index] = balance[index-1] - principal_payment[index];
     interest_payment[index] = std::round(interest_rate*balance[index] / 0.01) * 0.01;
     total_payment[index] = principal_payment_per_period + interest_payment[index];
   }
   ...
   principal_payment[total_periods - 1] = balance[total_periods - 2]/2;
   balance[total_periods-1] = principal_payment[total_periods - 1];
   total_payment[total_periods-1] = principal_payment[total_periods - 1] + interest_payment[total_periods-1];
